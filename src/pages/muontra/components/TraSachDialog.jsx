...
import {
  formatCurrency, getOverdueDays, getTodayValue, OVERDUE_FINE_PER_BOOK_PER_DAY,
} from "@/pages/muontra/muonTraUtils";
import { formatDisplayDate } from "@/utils/dateUtils";

function TraSachDialog({ onReturned, row }) {
  const canReturn = row.TrangThai === "Đang mượn" || row.TrangThai === "Quá hạn";
  ...
  const totalBooks = details.reduce((total, detail) => total + detail.SoLuong, 0);
  const overdueDays = getOverdueDays(row.HanTra, returnDate);
  const overdueFine = overdueDays * totalBooks * OVERDUE_FINE_PER_BOOK_PER_DAY;
        ...
        <div className="space-y-5 px-6 py-5">
          ...
          <div className={`rounded-xl border p-4 ${overdueDays ? "border-rose-200 bg-rose-50" : "border-emerald-200 bg-emerald-50"}`}>
            <div className="grid gap-2 text-sm md:grid-cols-2">
              ...
              <span>Mức phạt</span><strong className="text-right">{formatCurrency(OVERDUE_FINE_PER_BOOK_PER_DAY)} / cuốn / ngày</strong>
