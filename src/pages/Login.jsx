...
import { getCurrentUser, login, USER_ROLES } from "@/utils/auth";
...
function Login() {
  ...
  const handleSubmit = (event) => {
    ...
    const fallbackPath = user.role === USER_ROLES.MANAGER ? "/" : "/sach";
    const isForbiddenPath =
      user.role === USER_ROLES.LIBRARIAN &&
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#fffaf4] px-4 py-8 text-slate-800">
      <section className="grid w-full max-w-5xl overflow-hidden rounded-2xl border border-orange-100 bg-white shadow-2xl shadow-orange-950/10 md:grid-cols-[1fr_1fr]">
        ...
        <form className="flex flex-col justify-center p-8 md:p-12" onSubmit={handleSubmit}>
          ...
          <Button className="h-11 rounded-xl bg-orange-500 font-bold shadow-lg shadow-orange-500/20 hover:bg-orange-600" type="submit">
            Đăng nhập
          </Button>
          <p className="mt-5 text-center text-xs font-semibold text-slate-500">
            Quản lý: <span className="text-slate-800">admin / 123456</span>
            <br />
            Thủ thư: <span className="text-slate-800">thuthu / 123456</span>
          </p>
        </form>
      </section>
    </main>
