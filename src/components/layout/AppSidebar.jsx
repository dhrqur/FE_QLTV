...
const navItems = [
  { label: "Dashboard", href: "/", icon: LayoutDashboard, roles: [USER_ROLES.MANAGER] },
...
function AppSidebar({ collapsed = false }) {
  ...
  const visibleNavItems = navItems.filter((item) => {
    ...
    return !managerOnlyPaths.includes(item.href) || user?.role === USER_ROLES.MANAGER;
  });
  ...
  return (
    ...
    >
      ...
      <nav className={cn("flex-1 space-y-1 overflow-y-auto py-4", collapsed ? "px-3" : "px-3")}>
