    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-6xl p-0" onOpenAutoFocus={(event) => event.preventDefault()}>
        ...
        <DialogFooter className="border-t border-slate-100 bg-slate-50 px-6 py-4">
          ...
          <Button className="bg-orange-500 font-bold hover:bg-orange-600" form={formId} type="submit">
...
function FormField({ column, error, isEdit, row, rows }) {
  ...
  return (
    <label className="space-y-2 text-sm font-bold text-slate-700">
      ...
      {renderInput({ column, defaultValue, error, isLockedPrimaryKey, isRequired })}
...
function renderInput({ column, defaultValue, error, isLockedPrimaryKey, isRequired }) {
    ...
    return (
      <select
        className={`h-10 w-full rounded-lg border px-3 text-sm font-medium outline-none transition focus:ring-3 ${inputErrorClass} ${
          isLockedPrimaryKey ? "bg-slate-50 text-slate-500" : "bg-white text-slate-700"
        }`}
        defaultValue={defaultValue}
        disabled={isLockedPrimaryKey}
        name={column.key}
      >
